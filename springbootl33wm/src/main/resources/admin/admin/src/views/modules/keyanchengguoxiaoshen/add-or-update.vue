<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="成果名称" prop="chengguomingcheng">
          <el-input v-model="ruleForm.chengguomingcheng" 
              placeholder="成果名称" clearable  :readonly="ro.chengguomingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="成果名称" prop="chengguomingcheng">
              <el-input v-model="ruleForm.chengguomingcheng" 
                placeholder="成果名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="成果类型" prop="chengguoleixing">
        ...
        <div v-else>
          <el-form-item class="input" label="成果类型" prop="chengguoleixing">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.chengguotupian" label="成果图片" prop="chengguotupian">
          <file-upload
          tip="点击上传成果图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.chengguotupian?ruleForm.chengguotupian:''"
          @change="chengguotupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.chengguotupian" label="成果图片" prop="chengguotupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.chengguotupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="成果详情" prop="chengguoxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.chengguoxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.chengguoxiangqing" label="成果详情" prop="chengguoxiangqing">
                    <span v-html="ruleForm.chengguoxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	chengguotupian : false,
	faburiqi : false,
	chengguoxiangqing : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chengguoleixing = true;
          ...
          if(o=='chengguotupian'){
            this.ruleForm.chengguotupian = obj[o];
	    this.ro.chengguotupian = true;
            continue;
          }
          if(o=='faburiqi'){
	    ...
	    this.ro.faburiqi = true;
          ...
          if(o=='chengguoxiangqing'){
            this.ruleForm.chengguoxiangqing = obj[o];
	    this.ro.chengguoxiangqing = true;
            continue;
          }
          if(o=='jiaoshigonghao'){
    ...
    info(id) {
      this.$http({
        url: `keyanchengguoxiaoshen/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.chengguoxiangqing = this.ruleForm.chengguoxiangqing.replace(reg,'../../../springbootl33wm/upload');
	...
	if(this.ruleForm.chengguotupian!=null) {
		this.ruleForm.chengguotupian = this.ruleForm.chengguotupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `keyanchengguoxiaoshen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.keyanchengguoxiaoshenCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    chengguotupianUploadChange(fileUrls) {
	this.ruleForm.chengguotupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{

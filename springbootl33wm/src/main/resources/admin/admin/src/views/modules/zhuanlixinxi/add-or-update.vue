<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="专利编号" prop="zhuanlibianhao">
            <el-input v-model="ruleForm.zhuanlibianhao" 
                placeholder="专利编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.zhuanlibianhao" label="专利编号" prop="zhuanlibianhao">
              <el-input v-model="ruleForm.zhuanlibianhao" 
                placeholder="专利编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="专利名称" prop="zhuanlimingcheng">
          <el-input v-model="ruleForm.zhuanlimingcheng" 
              placeholder="专利名称" clearable  :readonly="ro.zhuanlimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="专利名称" prop="zhuanlimingcheng">
              <el-input v-model="ruleForm.zhuanlimingcheng" 
                placeholder="专利名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="专利类型" prop="zhuanlileixing">
          <el-select :disabled="ro.zhuanlileixing" v-model="ruleForm.zhuanlileixing" placeholder="请选择专利类型">
            <el-option
                v-for="(item,index) in zhuanlileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="专利类型" prop="zhuanlileixing">
	      <el-input v-model="ruleForm.zhuanlileixing"
                placeholder="专利类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
    ...
    return {
      ...
      type: '',
      ro:{
	zhuanlibianhao : false,
	wenjian : false,
	zhuanlimingcheng : false,
	zhuanlileixing : false,
	beizhu : false,
	dengjishijian : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='zhuanlibianhao'){
            this.ruleForm.zhuanlibianhao = obj[o];
	    this.ro.zhuanlibianhao = true;
            continue;
          }
          if(o=='wenjian'){
	    ...
	    this.ro.wenjian = true;
          ...
          if(o=='zhuanlimingcheng'){
            this.ruleForm.zhuanlimingcheng = obj[o];
	    this.ro.zhuanlimingcheng = true;
            continue;
          }
          if(o=='zhuanlileixing'){
            this.ruleForm.zhuanlileixing = obj[o];
	    this.ro.zhuanlileixing = true;
            continue;
          }
          if(o=='beizhu'){
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.zhuanlileixingOptions = "发明专利,实用新型专利,外观设计".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zhuanlixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhuanlixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhuanlixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
